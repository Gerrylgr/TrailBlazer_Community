...
void Preprocess::vanjee722_handler(const sensor_msgs::msg::PointCloud2::UniquePtr &msg)
{
  pl_surf.clear();
  pl_corn.clear();
  pl_full.clear();

  pcl::PointCloud<vanjee_722_ros::Point> pl_orig;
  ...
  double t_min = pl_orig.points.front().timestamp;
  double t_max = t_min;
  ...
  for (int i = 0; i < plsize; ++i)
  {
    ...
    // 兼容 ring = 0~31 或 ring = 1~32
    const bool ring_zero_based = src.ring < N_SCANS;
    const bool ring_one_based = src.ring >= 1 && src.ring <= N_SCANS;
    if (!ring_zero_based && !ring_one_based) continue;
    ...
    if (std::fabs(t_min) > 1e8 && dt < 1.0)
    {
      // absolute seconds -> relative ms
      offset_ms = static_cast<float>((src.timestamp - t_min) * 1000.0);
    }
    else if (t_max <= 1.0)
    {
      // relative seconds -> ms
      offset_ms = static_cast<float>(src.timestamp * 1000.0);
    }
    else if (t_max <= 200.0)
    {
      // already ms
      offset_ms = static_cast<float>(src.timestamp);
    }
    else if (t_max <= 200000.0)
    {
      // us -> ms
      offset_ms = static_cast<float>(src.timestamp / 1000.0);
    }
    ...
    {
      // ns -> ms
      offset_ms = static_cast<float>(src.timestamp / 1000000.0);
    }

    if (offset_ms < 0.0f || offset_ms > 200.0f) continue;

    pt.curvature = offset_ms;
    ...
    const sensor_msgs::msg::PointCloud2::UniquePtr &msg)
{
  ...
  const std::size_t plsize = pl_orig.points.size();
  if (plsize == 0)
  {
    return;
  }
